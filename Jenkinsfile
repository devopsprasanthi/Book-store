pipeline {
    ...
    environment {
        ...
        registry = "devopsprasanthi/book-store"
    }
    stages{
        stage('Docker Build') {
            steps {
                sh 'docker build -t devopsprasanthi/book-store .'
              }
          }
        stage('Docker Login & Push') {
            steps {
		sh 'echo $DOCKERHUB_CREDENTIALS_PSW | docker login -u $DOCKERHUB_CREDENTIALS_USR --password-stdin'
                sh 'docker push devopsprasanthi/book-store'
              }
          }
        stage('Remove Docker Images') {
            steps {
                sh 'docker rmi -f devopsprasanthi/book-store'
              }
          }
